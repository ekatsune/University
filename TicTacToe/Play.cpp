 ...
 * - CheckDiagonals()
 * - CheckDraw()
 * - перегрузки операторов `operator[]`.
 * @author Ekatsune
 * @see Play
 */
...
constexpr char Play::empty_field;
 ...
 *
 * @param size_of_field - размер стороны игрового поля (N×N).
 */
 ...
 * @param count - индекс строки (начиная с 0), которая будет выведена.
 */
 ...
 * @param[out] winner - символ победителя, если найден (`X` или `O`).
 */
 ...
 * чьей-то победы, полю класса `winner` присваивается значение выигравшего пользователя.
 * @param winner - символ победителя, если найден (`X` или `O`).
 * @return Булевое значение - выполнено/не выполнено условие победы (`true` - если да, `false` - если нет).
 */
 ...
 * @param index - индекс строки (0 ≤ index < size).
